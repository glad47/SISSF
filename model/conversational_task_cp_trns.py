...
class ConversationalModule(nn.Module):
    ...
    def initEmbeddings(self):
        if self.embedding is not None:
            ...
            self.embedding = nn.Embedding(self.voc_size, self.embedding_size, self.pad_token_id)
    ...
    def _build_model(self):
        ...
        self.lin1 = nn.Linear(self.user_embed_dim, self.embedding_size)
        self.lin2 = nn.Linear(self.user_embed_dim, self.embedding_size)
        # loss function 
        self.conv_loss = nn.CrossEntropyLoss(ignore_index=self.pad_token_id)
        self.output_copy = nn.Linear(self.embedding_size, self.voc_size)  # Project to the number of items
        self.copy_projection = nn.Linear(self.embedding_size  * 2, self.embedding_size )
        self.output_projection_social = nn.Linear(self.user_embed_dim, self.embedding_size ) 
        self.output_projection = nn.Linear(self.embedding_size, self.voc_size)
        self.copyTrans = RecommenderTransformer(embed_dim= self.embedding_size,n_items= self.voc_size,ffn_dim=self.ffn_dim ,dropout=self.dropout ,n_heads=self.n_heads ,attention_dropout=self.attention_dropout ,n_layers=self.n_layers ,device= self.device)
    ...
    def get_copy_social_force_teaching(self, dialog_latent, social_reps):
        ...
        decode_atten_social_reps = decode_atten_social_reps.view(bs, seq_len, self.embedding_size) # (bs, seq_len, ffn_size)
    ...
    def force_teaching(self, response,encoder_output, encoder_mask,conv_history_embeddings, social_embeddings,social_reps, user_model, sampling_prob=0.7):
        ...
        bs, n_social, _ = social_reps.shape
        copyedRep = self.copyTrans(latent, social_embeddings.unsqueeze(1).repeat(1, seq_len, 1), None, True)
            
        logits = copyedRep *  self.decoder_token_prob_weight.to(self.device)
    ...
    def greedy_selection_v1(self, response,encoder_output, encoder_mask,conv_history_embeddings, social_embeddings, social_reps, user_model):
        ...
        for _ in range(self.response_truncate):
            ...
            bs, n_social, _ = social_reps.shape
            copyedRep = self.copyTrans(last_latent.squeeze(1), social_embeddings, None)
            
            
        

            logit = copyedRep *  self.decoder_token_prob_weight.to(self.device).squeeze(1)
    ...
    def greedy_selection_v2(self, response,encoder_output, encoder_mask,conv_history_embeddings, social_embeddings, social_reps, user_model):
        batch_size = response.shape[0]
        inp = response[:, 1:].to(self.device)
        start = self.START.detach().expand(batch_size, 1).to(self.device)
        inputs = torch.cat((start, inp), dim=-1).long().to(self.device)        
    ...
    def forward(self, user_model, batch):
        ...
        encoder_output = user_model.conversation_encoder.get_encoder_rep(batch)
        ...
        social_embeddings = self.lin1(social_embeddings).to(self.device)
  ...
  # def force_teaching(self, response,encoder_output, encoder_mask,conv_history_embeddings, social_embeddings,social_reps, user_model, sampling_prob=0.7):
    ...
    #         bs, seq_len, _ = last_latent.shape
    #         bs, n_social, _ = social_reps.shape
    #         copyedRep = self.copyTrans(last_latent.unsqueeze(1) )
    #         # mask = (social_reps.sum(dim=-1, keepdim=True) == 0).squeeze(2).to(self.device)
    #         # social_reps_transformed = self.output_projection_social(social_reps).to(self.device)
        
            

    #         dot_prod = last_latent.bmm(social_embeddings.transpose(1, 2)).to(self.device) # (bs, seq_len, dim)
    #         # where to apply attention 
    #         # transfor user_emd to ffn_emd
    #         # attn_mask = mask.unsqueeze(1).expand(-1, seq_len, -1).to(self.device) # (bs, seq_len, nb_review)
    #         # dot_prod.masked_fill_(attn_mask.bool(), neginf(dot_prod.dtype)).to(self.device) # (bs, seq_len, nb_review)
    ...
    #         weight = F.softmax(dot_prod, dim=-1).type_as(social_embeddings).to(self.device) # (bs, seq_len, n_social)

    #         decode_atten_social_reps = weight.bmm(social_embeddings).to(self.device) # (bs, seq_len, ffn_size)
    #         # decode_atten_social_reps = decode_atten_social_reps.view(bs, seq_len, self.embedding_size).to(self.device) # (bs, seq_len, ffn_size)
    #         copy_latent= self.copy_projection(torch.cat([decode_atten_social_reps ,last_latent],-1)).to(self.device)
    #         #logits = self.output(latent)
    #         con_logits = self.output_copy(copy_latent).to(self.device)   #F.linear(copy_latent, self.embeddings.weight)
