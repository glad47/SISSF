...
with open('input/inspired/conversations.json', 'r', encoding='utf-8') as file:
...
ratings = pd.read_excel('input/inspired/ratings.xlsx')
...
unique_movies = set()
unique_users = set()
allConvsIda= [con['conversationId'] for con in conversations] 
allConv = conversations  
movieLikedDict = {}
...
def createUniqueMapping(unique_movies, unique_users):
    ...
    with open('input/inspired/user_list.txt') as f:
            check = False
    ...
    with open('input/inspired/item_list.txt') as f:
        check = False
        for l in f.readlines():
            if check :
                if len(l) > 0:
    ...
    for conversation in conversations:
        ...
        if isinstance(conversation['initiatorQuestions'], dict):
            for movie_id, details in conversation['initiatorQuestions'].items():
                ...
                initiator_mentioned_entities.append(movie_to_index[int(movie_id)])  
        ...
        if isinstance(conversation['respondentQuestions'], dict):
            for movie_id, details in conversation['respondentQuestions'].items():
                ...
                respondent_mentioned_entities.append(movie_to_index[int(movie_id)])
                ...
                for word in msg['text']: 
                    if '@' in word and word[1:].isdigit():
                        ...
                        if movie_id in movie_to_index:
                            ...
                            msg['entities'].append(movie_to_index[movie_id])
...
def statics_data(users_set, user_count_dict):
    print("statics_data")
    for conversation in conversations:
        index = allConvsIda.index(conversation['conversationId'])
...
def create_new_data(user_count_dict, newConversations):
...
def process_data(train_conv,movieLikedDict, userRelations, token_freq, recommenderSeekerTracking):
    token_mapping["__pad__"] =0 
    token_mapping["__start__"] =1 
    token_mapping["__end__"] =2 
    token_mapping["__unk__"] =3 
    print("process_data")
            ...
            for token in msg['text']:
                    try:
                        ...
                        if random.random() < 0.01:  # 1% chance
                            print("Token not added due to 1% probability function.")
                        else:
                            new_id = len(token_mapping)
                            token_mapping[token] = new_id
                            token_freq[new_id] = 1
            ...
            for user2 in userRelations.get(user1):
                ...
                if len(likedItemUser1.intersection(likedItemUser2)) > 0 :
                    ...
                    for item in commonItems:
                        ...
                        user2Ratings += ratings.loc[user1,  movie_to_index[int(item)]]
...
def create_interaction_dataset(interaction_matrix, user_interactions,user_ratings,item_interactions, item_ratings):
    ...
    with open('outputSISSFInspired/inspired_interactions.pickle', 'wb') as f:
...
statics_data(users_training, users_training_count) 
create_new_data(users_training_count, newConversations) 
...
with open('outputSISSFInspired/train_data.json', 'w') as json_file:
    json.dump(newTrain, json_file, indent=4)

with open('outputSISSFInspired/valid_data.json', 'w') as json_file:
    json.dump(newTest, json_file, indent=4)    

with open('outputSISSFInspired/test_data.json', 'w') as json_file:
    json.dump(newVal, json_file, indent=4)


with open('outputSISSFInspired/token2id.json', 'w') as json_file:
    json.dump(token_mapping, json_file, indent=4)

with open('outputSISSFInspired/token_freq.json', 'w') as json_file:
