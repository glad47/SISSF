...
class SISSF:
        ...
        if not self.cli_args.test_mode :
            for epoch in range(self.start_epoch, self.cli_args.epochs + 1):
                ...
                if self.cli_args.mode == 'semantic':
                    ...
                    if  score > 0.0400 or test_score > 0.0400:
                        self.best_score = score
                        self.endure_count = 0
                        self.saveModel(epoch, False)
                    # else:
            ...
            singleConversation = []
            currentConv = 0
            for index, item in enumerate(self.dataset.valid_data):
                
                if len(singleConversation) == 0:
                    singleConversation.append(item)
                    currentConv = item['conversationId']
                elif item['conversationId'] == currentConv:
                    singleConversation.append(item)
                else:
                    aggergatedConv.append(singleConversation)
                    singleConversation = []
                    singleConversation.append(item)
                    currentConv = item['conversationId']
            aggergatedConv.append(singleConversation)
            ...
            for index, items in enumerate(aggergatedConv):
                self.dataset.test_data =SingleDataset(items)
                self.initTestDataloaders()
                testMetrics_t = self.doTesting(0)
                score = self.logMetrics(0, 'test', testMetrics_t) 
                new_key_value = {'conversationId': int(items[0]['conversationId'])}

                # Create a new OrderedDict with the new key-value pair first
                new_dict = OrderedDict(list(new_key_value.items()) + list(score.items()))
            ...
            df.to_excel('anaylsis_valid.xlsx', index=False)        
    ...
    def mainV0(self):
        ...
        if not self.cli_args.test_mode :
            ...
            singleConversation = []
            currentConv = 0
            for index, item in enumerate(self.dataset.test_data):
                
                if len(singleConversation) == 0:
                    singleConversation.append(item)
                    currentConv = item['conversationId']
                if item['conversationId'] == currentConv:
                    singleConversation.append(item)
                else:
                    aggergatedConv.append(singleConversation)
                    singleConversation = []
            ...
            for index, items in enumerate(aggergatedConv):
                ...
                testMetrics_t = self.doTesting(0)
                score = self.logMetrics(0, 'test', testMetrics_t) 
                new_key_value = {'conversationId': items[0]['conversationId']}

                # Create a new OrderedDict with the new key-value pair first
                new_dict = OrderedDict(list(new_key_value.items()) + list(original_dict.items()))
                new_dict = dict(new_dict)
                table.append(new_dict)
            # Convert list of dictionaries to DataFrame
            df = pd.DataFrame(table)
            ...
            # Export DataFrame to Excel file
            df.to_excel('anaylsis_test.xlsx', index=False)    
